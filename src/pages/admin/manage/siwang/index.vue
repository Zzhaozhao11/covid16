...
*/
    ...
    */
    async search() {
      ...
      let result = await this.$http.admin.ReqSelectDeadByName(this.tempSearchObj.username);
      ...
      for (const user of result.data.data) { //遍历数组
        let { id, name, personId, gender, houseAddress, deadDate } = user;
        let newUser = { id, name, personId, gender, houseAddress, deadDate, status: '无' };
    ...
    */
    async getUsers(page = 1) {
      ...
      this.loading = true;
      this.users = [];
      const result = await this.$http.admin.ReqSelectAllDead(limit, page);
      this.loading = false;
      if (!result.data.data) {
        this.$message.error('错误');
        this.loading=false;
